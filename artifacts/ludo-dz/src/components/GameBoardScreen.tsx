  ...
  return (
    ...
    <motion.div
      onClick={canTap ? () => {
        ...
        if (!isNeon && !isClassic && !isDz) playPrimaryAction();
        onRoll();
      } : undefined}
    ...
    <motion.div
      ...
      <motion.div
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          ...
          <button onClick={() => { isNeon ? playNeonClick() : isClassic ? playClassicClick() : isDz ? playDzClick() : playIconTap(); onClose(); }}
...
export function GameBoardScreen({ config, lang, boardStyle, onBack }: Props) {
  ...
  const handlePieceClick = useCallback((pid: string) => {
    ...
    if (currentGame.movable.includes(pid)) {
      isNeon ? playNeonClick() : isClassic ? playClassicClick() : isDz ? playDzPawnSelect() : playSelection();
      triggerMove(pid);
      return;
    }
